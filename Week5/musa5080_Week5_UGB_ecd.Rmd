...
ggplot()+
  ...
  geom_sf(data = uga, fill = "transparent", color = "white", linetype = 5, lwd = 0.5)+ # urban growth area
  theme_void()
```
...
tracts17 <- 
  tracts17 %>%
  mutate(popden = ifelse(TotalPop > 0, TotalPop / (area_sqFt / 27878400), 0))
...
# Calculate quantiles for class breaks
breaks_quantiles <- classIntervals(tracts17$popden, n = 5, style = "quantile")

# Define colors using ColorBrewer palette - matching the number of breaks
colors <- brewer.pal(n = 5, name = "YlGn")
...
# Create improved map with custom range labels for legend
map_improved <- ggplot() +
  geom_sf(data = tracts17, aes(fill = cut(popden, breaks = breaks_quantiles$brks, include.lowest = TRUE)), color = "lightgrey") +
  scale_fill_manual(values = colors,
...
# Create improved map with custom range labels for legend
map_improved <- ggplot() +
  geom_sf(data = tracts17, aes(fill = cut(popden, breaks = breaks_quantiles$brks, include.lowest = TRUE)), color = "grey") +
  scale_fill_manual(values = c("#BBD3D9","#87A6AE","#507984","#274F59","#0E2C33"),
                    name = "Population Density",
                     labels = labels) +
  geom_sf(data = uga, fill = NA, color = "white", linetype = 1, lwd = 0.5) +  # Use NA for fill to make the boundaries transparent
  labs(title = "Population Density of Census Tracts in Lancaster County") +
...
## Install tmap 4 - install from remote
...
```{r buffers}
outsideBuffer <-
  st_buffer(uga_union, 660) %>% # positive - outside, negative - inside
...
insideBuffer <- 
  ...
  st_difference(uga_union, .) %>% # . means flip
...
insidecensus <- st_intersection(tracts17, insideBuffer)
outsidecensus <- st_intersection(tracts17, outsideBuffer)

alltract <- rbind(insidecensus, outsidecensus) %>%
  group_by(Legend) %>%
  summarize('Population Density' = mean(popden, na.rm = TRUE))
