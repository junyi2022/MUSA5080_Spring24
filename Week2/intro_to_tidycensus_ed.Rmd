...
```{r do_exercise, cache = TRUE}
allACS <- allACS %>%
  mutate(change_total_pop = total_pop.2020-total_pop.2016, 
         change_percent_pop = (total_pop.2020-total_pop.2016)/total_pop.2016,
         change_percent_HU = (total_HU.2020-total_HU.2016)/total_HU.2016)
...
ggplot(allACS)
#ggplot(data=<DATA>) make a canvas
...
ggplot(allACS)+
  ...
  labs(
    ...
    x="Change in Med HH Income (2020 dollars)", 
       y="Number of tracts")+
  facet_wrap(~mtAiry, scales = "free")+ #2 plots
  theme_minimal()
...
ggplot(allACS)+
  geom_point(aes(x =med_HH_Income.2016 * 1.08, # 1.08 is inflation factor
