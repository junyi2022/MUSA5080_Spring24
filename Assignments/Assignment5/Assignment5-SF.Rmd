---
title: "Space-Time Prediction of August Bike Share Demand in San Francisco"
author: "Junyi Yang"
date: "Apirl, 2024"
output:
  html_document:
...
Let's read in the month of August 2023's data from (Bay Wheel's AWS site)[https://s3.amazonaws.com/baywheels-data/index.html]

```{r read_dat }
dat <- read.csv("data/202308-baywheels-tripdata.csv")
```
...
# dat_clipped <- st_intersection(dat, sfTracts)
# dat2_withgeo <- dat2 %>% 
...
dat_census <- st_join(dat2 %>% 
  filter(is.na(start_lng) == FALSE &
           ...
           is.na(end_lat) == FALSE &
           is.na(end_lng) == FALSE) %>%
  st_as_sf(., coords = c("start_lng", "start_lat"), crs = 4326),sfTracts %>%
          st_transform(crs=4326),
        join=st_intersects,
              left = TRUE) %>%
...
weather.Panel <- 
  riem_measures(station = "SFO", date_start = "2023-08-01", date_end = "2023-08-31") %>%
...
grid.arrange(
  ...
  top="Weather Data - sf ORD - May, 2018")
```
...
ggplot(dat_census %>%
  ...
  labs(title="Bike share trips per hr. sf, May, 2018",
       x="Date", 
       y="Number of trips")+
  plotTheme
```
...
dat_census %>%
  ...
  ggplot()+
  geom_histogram(aes(mean_trips), binwidth = 3)+
  labs(title="Mean Number of Hourly Trips Per Station. sf, May, 2018",
       x="Number of trips", 
       y="Frequency")+
...
ggplot(dat_census %>%
  ...
  labs(title="Bike share trips per hr by station. sf, May, 2018",
...
ggplot(dat_census %>% mutate(hour = hour(started_at)))+
  ...
  labs(title="Bike share trips in sf, by day of the week, May, 2018",
       x="Hour", 
       y="Trip Counts")+
     plotTheme
...
ggplot(dat_census %>% 
  ...
  labs(title="Bike share trips in sf - weekend vs weekday, May, 2018",
       x="Hour", 
       y="Trip Counts")+
     plotTheme
```
...
ride.panel <- 
  ...
  group_by(interval60, start_station_id, start_station_name, Origin.Tract, start_lng, start_lat) %>%  # Group by spatial-time and station variables
  ...
  filter(!is.na(start_station_id)) %>%  # Filter out rows with missing from_station_id values
...
week_predictions %>% 
    ...
    ggplot(aes(interval60, Value, colour=Variable)) + 
      ...
      labs(title = "Predicted/Observed bike share time series", subtitle = "sf; A test set of 2 weeks",  x = "Hour", y= "Station Trips") +
      plotTheme
```
